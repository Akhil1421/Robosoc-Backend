...
let deleteproject = ()=>{
    deleteDiv[0].style.display = "none"
    fetch(`/services/projects?id=${idOfprojectToBeDeleted}`,{
        "method" : "DELETE"
    })
    .then((res)=>{
        if(!res.ok){
...
const fetchData = () => {
    fetch("/services/projects", { method: "GET" })
        .then((res) => {
            if(!res.ok){
                if(res.status===403){
                    throw new Error('Access to requested resource is for admin only. Login as admin to access the resource')
        ...
        .then((res) => {
            ...
            projectsData.forEach((data) => {
                ...
                <div class="single-project">
                    ...
                    <div class="options">
                    <!--Edit--icon-->                        
                        <div onclick="handleEdit('${data._id}', '${data.name}', '${data.description}','${data.image}')" id="edit">
